<template>
  ...
  >
    ...
    <div class="p-1">
      <h2 class="text-white text-xs font-sans mb-2 mt-2">{{ tvshow.name }}</h2>
      <!-- Star Rating -->
      <template v-if="!simple">
        <div class="flex mb-1">
          <template v-for="star in maxStars">
            <svg
              v-if="star <= tvshow.vote_average / 2"
              fill="currentColor"
              viewBox="0 0 24 24"
        ...
        <p class="text-gray-400 text-[10px]">
          {{ formattedVoteAverage }}% | {{ formattedDate }}
        </p>
...
const maxStars = 5;
const formattedVoteAverage = computed(() => props.tvshow.vote_average * 10);
const formattedDate = computed(() =>
  new Date(props.tvshow.first_air_date).toLocaleDateString()
);

const goToDetail = () => {
  router.push(`/tv/${props.tvshow.id}`);
};
</script>
